...
import { parseTaskIdentifier } from './task'
import type {
  ActionType,
  ArgumentType,
  CLIArgumentType,
...
function isCLIArgumentType(type: ArgumentType<any>): type is CLIArgumentType<any> {
...
export class SimpleTaskDefinition implements TaskDefinition {
  ...
  constructor(
    ...
    this._task = task
    this.action = () => {
      throw new types.types.HardhatError('ERRORS.TASK_DEFINITIONS.ACTION_NOT_SET', {
        taskName: this._task,
      })
    }
  }
   ...
   */
  public setAction<TaskArgumentsT extends TaskArguments>(action: ActionType<TaskArgumentsT>) {
  ...
  public addParam<T>(
    ...
    if (type === undefined) {
      ...
      if (typeof defaultValue !== 'string') {
        throw new types.HardhatError('ERRORS.TASK_DEFINITIONS.DEFAULT_VALUE_WRONG_TYPE', {
          paramName: name,
          taskName: this.name,
        })
      }
  ...
  private _validateNotAfterVariadicParam(name: string) {
    if (this._hasVariadicParam) {
      throw new types.HardhatError('ERRORS.TASK_DEFINITIONS.PARAM_AFTER_VARIADIC', {
        paramName: name,
        taskName: this.name,
      })
    }
  ...
  private _validateNameNotUsed(name: string) {
    if (this._hasParamDefined(name)) {
      throw new types.HardhatError('ERRORS.TASK_DEFINITIONS.PARAM_ALREADY_DEFINED', {
        paramName: name,
        taskName: this.name,
      })
    }
    ...
    if (Object.keys(HARDHAT_PARAM_DEFINITIONS).includes(name)) {
      throw new types.HardhatError('ERRORS.TASK_DEFINITIONS.PARAM_CLASHES_WITH_HARDHAT_PARAM', {
        paramName: name,
        taskName: this.name,
      })
    }
   ...
   */
  private _validateNoMandatoryParamAfterOptionalOnes(name: string, isOptional: boolean) {
    if (!isOptional && this._hasOptionalPositionalParam) {
      throw new types.HardhatError('ERRORS.TASK_DEFINITIONS.MANDATORY_PARAM_AFTER_OPTIONAL', {
        paramName: name,
        taskName: this.name,
      })
    }
  ...
  private _validateParamNameCasing(name: string) {
    ...
    if (match === null) {
      throw new types.HardhatError(ERRORS.TASK_DEFINITIONS.INVALID_PARAM_NAME_CASING, {
        paramName: name,
        taskName: this.name,
      })
    }
  ...
  private _validateNoDefaultValueForMandatoryParam(
    ...
    if (defaultValue !== undefined && !isOptional) {
      throw new types.HardhatError('ERRORS.TASK_DEFINITIONS.DEFAULT_IN_MANDATORY_PARAM', {
        paramName: name,
        taskName: this.name,
      })
    }
  ...
  private _validateCLIArgumentTypesForExternalTasks(type: ArgumentType<any>) {
    ...
    if (!isCLIArgumentType(type)) {
      throw new types.HardhatError('ERRORS.TASK_DEFINITIONS.CLI_ARGUMENT_TYPE_REQUIRED', {
        task: this.name,
        type: type.name,
      })
    }
...
export class OverriddenTaskDefinition implements TaskDefinition {
  ...
  public addPositionalParam<T>(
    ...
    return this._throwNoParamsOverrideError('ERRORS.TASK_DEFINITIONS.OVERRIDE_NO_POSITIONAL_PARAMS')
  }
  ...
  private _throwNoParamsOverrideError(errorDescriptor: any): never {
    throw new types.HardhatError(errorDescriptor, {
      taskName: this.name,
    })
  }
}
