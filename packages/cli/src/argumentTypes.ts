...
import type { ArgumentType, CLIArgumentType } from './types'

export class HardhatError extends Error {
  constructor(code: string, msg: any, _error?: any) {
    super(code as string, msg)
  }
}
/**
 * String type.
...
export const string: CLIArgumentType<string> = {
   ...
   */
  validate: (_argName: string, value: any): void => {
    const isString = typeof value === 'string'

    if (!isString) {
      throw new Error('ERRORS.ARGUMENTS.INVALID_VALUE_FOR_TYPE')
    }
  },
}
...
export const float: CLIArgumentType<number> = {
   ...
   */
  validate: (argName: string, value: any): void => {
    const isFloatOrInteger = typeof value === 'number' && !Number.isNaN(value)
